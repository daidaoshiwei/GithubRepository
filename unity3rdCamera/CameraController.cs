...
public class CameraController : MonoBehaviour
{
    ...
    //摄像机初始偏移
    public Vector3 cameraInitOffset;
    ...
    //目标上一帧位置
    private Vector3 mTargetLastPos = Vector3.zero;
	int mCurrentValidTouchIndex = -1;
    ...
    // If target and/or camera is not set, try using fallbacks
    {
        ...
        //相机射线检测关闭人怪
        obstacleLayers = 1 << 0 | 1 << 1;
        groundLayers = 0 << 8 | 0 << 9 | 1 << 0;
#if UNITY_EDITOR
	rotationUpdateSpeed = 70;
	lookUpSpeed = 50;
#else
	rotationUpdateSpeed = 35;
	lookUpSpeed = 25;
#endif
    }
    ...
    // Verify setup, initialise bookkeeping
    {
        ...
        targetDistance = optimalDistance = maxDistance;// (camera.transform.position - target.transform.position).magnitude;
        EnableCamera(true);
    }

    public void EnableCamera(bool bEnable)
    {
        enabled = bEnable;
        if(enabled)
        {
            //初始化一些必要的位置
            camera.transform.position = target.transform.position + cameraInitOffset.normalized * maxDistance;
            camera.transform.LookAt(target.transform.position);
            mTargetLastPos = target.transform.position;
            //mCameraOriginPos = camera.transform.position;
    ...
    // The minimum clear radius between the camera and the target
    {
        get
        {
            ...
            float doubleCharacterRadius = 0;// Mathf.Max(target.bounds.extents.x, target.bounds.extents.z) * 2.0f;
    ...
    // Update camera position - specifics are delegated to camera mode functions
    {
		...
		if(Input.GetMouseButtonDown(0))
		{
			...
			{
				mCurrentValidTouchIndex = 0;
			}
		}
        else if (Input.GetMouseButton(0))
        {
			if(mCurrentValidTouchIndex != -1)
			{
				FreeUpdate(Input.GetAxis("Mouse X"),Input.GetAxis("Mouse Y"));
			}
        ...
        for (int i = 0; i < Input.touchCount; i++)
        {
			...
			{
				if (EventSystem.current && EventSystem.current.IsPointerOverGameObject(Input.GetTouch(i).fingerId))
				{
					print("触摸索引:" + i + " FingerID:"+ Input.GetTouch(i).fingerId +"当前点击在UI上");
				}
				else
				{
					if(mCurrentValidTouchIndex == -1)
						mCurrentValidTouchIndex = i;
				}
			}
			else if ((Input.GetTouch(i).phase == TouchPhase.Ended))
			{
				if(mCurrentValidTouchIndex == i)
					mCurrentValidTouchIndex = -1;
			}
		    else if ((Input.GetTouch(i).phase == TouchPhase.Moved))
            {
				...
				FreeUpdate(Input.GetTouch(i).deltaPosition.x,Input.GetTouch(i).deltaPosition.y);
                lastStationaryPosition = target.transform.position;
				break;
            }
	...
	void FreeUpdate(float xDelta,float yDelta)
    // Control the camera via the mouse
    {
        ...
        {
			rotationAmount = xDelta * rotationUpdateSpeed * Time.deltaTime;
            camera.transform.RotateAround(target.transform.position, Vector3.up, rotationAmount);
        }

		// Vertical rotation: Input.GetAxis("Mouse Y")


		rotationAmount = yDelta * -1.0f * lookUpSpeed * Time.deltaTime;
    ...
    {
        //Debug.LogWarning("[]:" + (camera.transform.position) + "        []:" + (target.transform.position) + "        []:" + (camera.transform.position - target.transform.position).normalized + "[]:" + targetDistance);
        ...
        Vector3 targetPosition = camera.transform.position + (target.transform.position - mTargetLastPos);
        camera.transform.position = target.transform.position + (targetPosition - target.transform.position).normalized * targetDistance;
        //camera.transform.position = targetPosition;// Vector3.Lerp(camera.transform.position, targetPosition, Time.deltaTime * distanceUpdateSpeed);
        mTargetLastPos = target.transform.position;
        camera.transform.LookAt(target.transform.position);
    }
